...
use Cassandra\Exception\DataTypeException;

/**
 * Converter into binary data type
 */
final class ConverterDataType {
		...
		switch($dataType['type']) {
			...
			case DataTypeEnum::TIMESTAMP:
				if (is_double($value) && preg_match('~^\d{10}(.\d+)?$~', $value)) {
					...
					throw new DataTypeException('Value of timestamp must have 13 digits.');
				}
	 ...
	 * Return binary uuid
	 * @param string $value
	 * @throws \Cassandra\Exception\DataTypeException
	 * @return string
	 */
	private static function getUUID($value) {
		if (!is_string($value)) throw new DataTypeException('Incoming value must be of type string.');
	 ...
	 * @return string
	 */
	private static function getList($values, array $valueType) {
		if ((array)$values !== $values) throw new DataTypeException('Incoming value must be of type array.');
	 ...
	 * @return string
	 */
	private static function getMap($values, array $keyType, array $valueType) {
		if ((array)$values !== $values) throw new DataTypeException('Incoming value must be of type array.');
		$data = pack('n', count($values));
	 ...
	 * @return string
	 */
	private static function getBigInt($value) {
		if (!is_int($value)) throw new DataTypeException('Incoming value must be of type integer.');
	 ...
	 * @return string
	 */
	private static function getCustom($value) {
	...
	private static function getBoolean($value) {
		if (!is_bool($value)) throw new DataTypeException('Incoming value must be of type boolean.');
	 ...
	 * @return string
	 */
	private static function getDecimal($value) {
		if (!is_numeric($value)) throw new DataTypeException('Incoming value must be numeric.');
	 ...
	 * @return string
	 */
	private static function getDouble($value) {
		if (!is_double($value)) throw new DataTypeException('Incoming value must be of type double.');
	...
	private static function getFloat($value) {
		if (!is_float($value)) throw new DataTypeException('Incoming value must be of type float.');
	 ...
	 * @return string
	 */
	private static function getInet($value) {
		if (!is_string($value)) throw new DataTypeException('Incoming value must be of type string.');
	...
	private static function getInt($value) {
		if (!is_int($value)) throw new DataTypeException('Incoming value must be of type int.');
		return pack('N', $value);
	}
}